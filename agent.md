...
- 默认上下文必须保持小而清晰；意图识别阶段只使用 skill 的 name、description 和 intent_codes，提槽阶段加载当前任务对应的业务 skill 正文和该 skill 声明的默认提槽 reference。
...
- reference 是 skill 的私有资料；默认提槽 reference 由 Router 在提槽阶段加载，其他 reference 只有已加载 skill 明确暴露，并且 planner 通过 `requested_references` 请求时，才允许加载。
